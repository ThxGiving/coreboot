...
/*
 * Values must match enum fan_profile in ramstage.c. The chip drives the CPU
 * fan autonomously (SmartFan IV off PECI), so the choice takes effect at the
 * next boot with no OS involvement.
 */
...
/* Read in mainboard ptt.c ("tpm_enable"): gates the fTPM (TPM2 ACPI table
   + firmware startup). */
...
/* Read in mainboard ptt.c ("tpm_clear"): one-shot; ptt.c sends TPM2_Clear at
   the next boot and then resets this back to No. */
...
/* Applied by mainboard ramstage.c ("sata_enable"): enables the 00:1f.2 SATA
   (AHCI) controller, off by default because the board boots from NVMe. */
