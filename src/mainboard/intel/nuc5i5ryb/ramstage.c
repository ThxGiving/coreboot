...
/* Fan temp source. NUVOTON_FAN_SOURCE_PECI0_CAL (0x1c) is an nct6779/91 encoding
   the nct6776 rejects (nct6775 driver: "Invalid temperature source 28"); use the
   CPU diode CPUTIN instead. */
...
/* SATA (00:1f.2) is off in the devicetree (NVMe boot); enable it from the
   "sata_enable" option. Runs before enumeration so the device gets scanned. */
