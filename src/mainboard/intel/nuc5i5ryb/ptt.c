...
/*
 * Intel PTT firmware-TPM 2.0 via the ME CRB at 0xFED70000. Broadwell PTT has no
 * CRB at the standard 0xFED40000 (that decode is the LPC window for a discrete
 * TIS TPM the board lacks -> all-1s); the ME exposes a TCG-PTP control area at
 * 0xFED70000 (offset 0, not the +0x40 locality layout) with the data buffer at
 * 0xFED70080. coreboot points the CRB command/response registers at that buffer
 * and publishes 0xFED70000 as the TPM2 table control area (Start Method 2); the
 * OS drives execution via the _SB.TPM _DSM start method (acpi/tpm.asl).
 */
...
/* Stage command in the buffer, re-arm (HCMD=0), execute (ctrl_start=1), poll.
   Returns the TPM RC (0 = ok), or 0xffffffff on timeout. */
...
/* TPM2_Startup(TPM_SU_CLEAR): per-boot startup, keeps keys (TPM 2.0 Part 3, CC 0x144). */
static void ptt_tpm_startup(void)
{
	static const uint8_t startup_clear[] = {
...
/* TPM2_Clear on the platform hierarchy with an empty-password session: resets
   ownership (clears SPS/EPS auth, evicts objects). Only works while the platform
   hierarchy still has its default empty auth, i.e. from firmware. One-shot via
   the "tpm_clear" option. TPM 2.0 Part 3, CC 0x126. */
...
static void ptt_setup(void)
{
	...
	/* One-shot clear: consume the request first, clear only if that stuck,
	   so a failed reset can't wipe the TPM on every boot. */
	if (get_uint_option("tpm_clear", 0)) {
		if (set_uint_option("tpm_clear", 0) == CB_SUCCESS)
			ptt_tpm_clear();
		else
			printk(BIOS_WARNING,
			       "PTT: tpm_clear set but not resettable; skipping to avoid a clear loop\n");
	}
}

/* Emit the TPM2 table (Start Method 2) pointing at the ME CRB, wired to the
   mainboard write_acpi_tables hook. The generic acpi_create_tpm2() is a no-op
   here since coreboot runs no TPM stack (tlcl_get_family() == TPM_UNKNOWN). */
				   ...
				   struct acpi_rsdp *rsdp)
{
	...
	printk(BIOS_DEBUG, "PTT: TPM2 table @0x%lx, control area 0x%x\n",
	       current, PTT_CRB_BASE);

	return current + header->length;
}

static void ptt_bs_setup(void *unused)
{
	ptt_setup();
}
