 ...
 * the chip - so the patch lands. fan[0] = FAN1 = the one CPU_FAN header.
 */
...
{
	/*
	 * Enable Intel PTT (firmware TPM in the CSE) via the HECI
	 * FW_FEATURE_SHIPMENT_OVERRIDE message. Must run in ramstage before FSP
	 * Silicon Init / End-of-Post, while HECI is still up. coreboot already has
	 * the CRB TPM driver (CRB_TPM @ 0xfed40000, HAVE_INTEL_PTT) but the ME
	 * ships with PTT off ("crb_tis_probe: Intel PTT is not active") so the CRB
	 * stays dead and no TPM2 table is built. If this flips the PTT state,
	 * cse_enable_ptt() triggers a global reset; the next boot has PTT active
	 * and a usable fTPM. No-op if PTT is already on or the CSE is disabled.
	 */
	cse_enable_ptt(true);
